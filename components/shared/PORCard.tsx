...
import { Award, Calendar, MapPin, ArrowUpRight } from "lucide-react";
    ...
    >
      <div className="relative bg-white/[0.02] border border-white/[0.06] rounded-2xl overflow-hidden hover:border-white/10 hover:bg-white/[0.04] transition-all duration-500">
        <div className="flex flex-col lg:flex-row">
          {/* Image Section */}
          <div className="relative lg:w-80 xl:w-96 flex-shrink-0 aspect-[16/10] lg:aspect-auto bg-gradient-to-br from-violet-500/20 to-purple-600/20">
            {por.imageUrl ? (
              <Image
                src={por.imageUrl}
                alt={por.organization}
                fill
                className="object-contain p-6 opacity-90 group-hover:opacity-100 group-hover:scale-[1.02] transition-all duration-700"
              />
            ) : (
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="w-20 h-20 rounded-2xl bg-violet-500/10 flex items-center justify-center">
                  <Award className="w-12 h-12 text-violet-400 opacity-60" />
                </div>
              </div>
            )}
          ...
          <div className="flex-1 p-6 lg:p-8 flex flex-col">
            ...
            {/* Organization */}
            <p className="text-sm text-violet-400/80 font-medium mb-2">
              {por.organization}
            </p>
            ...
            <div className="space-y-1.5 mb-4">
              {por.description.slice(0, 3).map((item, i) => (
            ...
            {por.achievements && por.achievements.length > 0 && (
              <div className="space-y-1.5 mb-4 py-4 border-t border-white/5">
                ...
                {por.achievements.slice(0, 2).map((achievement, i) => (
                  <div key={i} className="flex items-start gap-2 text-xs text-gray-400">
            ...
            <div className="flex flex-wrap gap-1.5 mt-auto pt-2">
              {por.technologies.slice(0, 6).map((tech) => (
                <span
                  key={tech}
